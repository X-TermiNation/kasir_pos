...
async function updateJumlahSatuan(
  id_cabang,
  id_gudang,
  id_barang,
  id_satuan,
  ...
  try {
    ...
    const satuan = barang.Satuan.find((s) => s._id.equals(id_satuan));
    if (!satuan) {
      throw new Error("Satuan not found in Barang");
    }

    if (action === "tambah") {
      realms.write(() => {
        satuan.jumlah_satuan += newJumlahSatuan;
      });
    ...
    console.log("Jumlah Satuan updated successfully");
    return satuan;
  } catch (error) {
    throw console.log("Failed to update jumlah_satuan: " + error);
  } finally {
    //realm.close()
  }
}
  ...
  try {
    ...
    await realms.write(() => {
      const trans = realms.create("Transaksi", {
        ...
        grand_total: data_transaksi.grand_total,
      });
...
//delivery
async function addDelivery(id_cabang, alamat_tujuan, no_telp_cust, id_trans) {
  try {
    realms.subscriptions.update((subs) => {
      subs.add(realms.objects("Delivery"));
    });

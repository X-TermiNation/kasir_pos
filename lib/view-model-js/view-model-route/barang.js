...
router.put(
  "/editjumlahsatuan/:id_barang/:id_cabang/:id_gudang/:id_satuan",
  async (req, res) => {
    const id_barang = Realm.BSON.ObjectId(req.params.id_barang);
    const id_cabang = Realm.BSON.ObjectId(req.params.id_cabang);
    const id_gudang = Realm.BSON.ObjectId(req.params.id_gudang);
    const id_satuan = Realm.BSON.ObjectId(req.params.id_satuan);

    const { jumlah_satuan, action } = req.body;
    try {
      const newSatuan = await updateJumlahSatuan(
        id_cabang,
        id_gudang,
        id_barang,
        id_satuan,
      ...
      if (!newSatuan) {
        console.log("stok satuan gagal ditambahkan");
      } else {
        console.log("stok satuan berhasil ditambahkan");
      }
      const redisKey = `data_satuan_` + id_barang;
      redis.del(redisKey, function (err, reply) {
        if (err) {
      ...
      //let satuan2 = await barang.Satuan
      let satuan2 = await SearchSatuanByIdBarang(
        id_cabang,
        id_gudang,
        id_barang
      );
      ...
      console.log("error update satuan:" + err);
    }
  }
);
